...
## Basic Usage
The simplest way to use the frost-page-title `addon` is to add __ember-frost-page-title__ to your `package.json` file and set your app's default title on the `APP['frost-page-title-default']` property in `config/environment.js`. Then __replace the title tag with the content-for helper__ the app's index.html, as the title will be set on build by the initializer.
```
// package.json
...
